...
class MainViewer(QWidget):
    ...
    def __init__(self, database, logger, parent=None):
        ...
        self.layout.addWidget(self.start_btn, 10, 4, 1, 1)
        self.circle_btn = IconButton(icon="QPushButton{border-image: url(./database/icon/circle.png)}", size=35)
        self.layout.addWidget(self.circle_btn, 10, 8, 1, 1)
...
class MyGraphicItem(QGraphicsPixmapItem):
    ...
    def update_pix_map(self, update_parameter=False):
        ...
        # 绘制圆
        radius = self.database.cfg['radius']
        center_x = self.database.cfg['center_x']
        center_y = self.database.cfg['center_y']
        h, w, _ = opencv_array.shape
        for i in range(3):
            cv2.circle(opencv_array, (int(w*center_x), int(h*center_y)), int(radius*min(h, w)), (0, 0, 255), 2)
