...
def mot_visualize_video(src_video_path, trk_seq_path, out_video_path, colors, width, height, fps, center_x, center_y, radius):
    """
    ...
    while src.isOpened():
        ...
        if not ret:
            break
        cv2.circle(frame, (int(user_cy), int(user_cx)), int(radius*min(height, width)), (0, 0, 255), 2)
        ...
        for _, worm_id, x, y, w, h, _, _, _, _ in frame_data:
            cv2.rectangle(frame, (y, x), (y + h, x + w), colors[(worm_id-1) % len(colors)], 2)  # 方框
            cv2.putText(frame, str(worm_id), (y, x), cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[(worm_id-1) % len(colors)], 2)  # ID号
...
def mot_visualize_image(out_image_dir, trk_seq_path, colors, width, height, center_x, center_y, radius):
    """
    ...
    for worm_id in range(1, trajectory_num+1):
        ...
        cv2.putText(trajectory_image_list[0], str(worm_id), (cy0, cx0), cv2.FONT_HERSHEY_SIMPLEX, 0.6, colors[(worm_id - 1) % len(colors)], 2)  # ID号
        for _, _, x, y, w, h, _, _, _, _ in id_data:
            cx, cy = int(x+w/2), int(y+h/2)
            cv2.line(trajectory_image_list[0], (cy0, cx0), (cy, cx), colors[(worm_id - 1) % len(colors)], 2)
    ...
    for i, trajectory_image in enumerate(trajectory_image_list):
        cv2.circle(trajectory_image, (int(width * center_x), int(height * center_y)), int(radius * min(height, width)), (0, 0, 255), 2)
        if i == 0:
...
def draw_math_results(json_path, out_image_dir, colors):
    ...
    speeds, swings, contact_times, contact_numbers, in_outs = [], [], [], [], []
    for i in range(1, data['number']+1):
        ...
        # draw_colors.append(f'rgb({color[0]}, {color[1]}, {color[2]})')
        speeds.append(id_dict['speed'])
        swings.append(id_dict['swing'])
        contact_times.append(int(id_dict['contact_time']))
        contact_numbers.append(int(id_dict['contact_num']))
        in_outs.append(int(id_dict['in_out']))
    _dict = {
        'Speed': speeds,
        'Swing': swings,
        'ContactTime': contact_times,
